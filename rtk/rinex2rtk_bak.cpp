...
void covariance_matrix(int n, MatrixXd &H_dd, MatrixXd &Q_dd, MatrixXd &W_dd)
{
    ...
    for (int i = 0; i < n; i++)
    {
        Q(3 * i, 3 * i) = Ccov;         //covariance of Pseudorange measurement
        Q(3 * i + 1, 3 * i + 1) = Ccov;
        Q(3 * i + 2, 3 * i + 2) = Pcov; //covariance of carrier phase measurement
    }
    //cout << "line: " << __LINE__ << endl;
    //compute cov of difference
    H_dd = MatrixXd::Zero(3 * n - 3, 3 * n);
    for (int i = 0; i < (n - 1); i++)
    {
        H_dd(3 * i, 3 * i) = 1;
        H_dd(3 * i, 3 * i + 3) = -1;
...
//double difference [dd_c12, dd_p12, dd_c23, dd_p23...]
void rtk_solver(vector<rtk_obs_t> &rtk_obs)
{
    int n = rtk_obs.size();
    cout << "rtk solver: " << n << endl;
    int rows = 3 * n - 3;//all the measurements converted to double difference
    ...
    for (int i = 0; i < (n - 1); i++)
    {
        ...
        double dd_c1 = (obs1.C1 - obs1.Cs1) - (obs2.C1 - obs2.Cs1);//double difference
        double dd_c2 = (obs1.C2 - obs1.Cs2) - (obs2.C2 - obs2.Cs2); //double difference
        double dd_p = (obs1.P - obs1.Ps) - (obs2.P - obs2.Ps); //double difference
        ...
        H.block(3 * i,     0, 1, 3) = (I1 - I2).transpose();
        H(3 * i, 3 + i) = L1_WAVELENGTH_GPS;

        H.block(3 * i + 1, 0, 1, 3) = (I1 - I2).transpose();
        H(3 * i + 1, 3 + i) = L2_WAVELENGTH_GPS;

        H.block(3 * i + 2, 0, 1, 3) = (I1 - I2).transpose();
    }
    //cout << "H: " << H << endl;
    ...
    MatrixXd HH = H.block(0, 0, rows, 3);
...
int main(int argc, char *argv[])
{
    ...
    int indexP1 = roh.getObsIndex("C1C");//Pseudorange on L1
    int indexC1 = roh.getObsIndex("L1C"); //carrier phase measurement on L1
    int indexC2 = roh.getObsIndex("L2W");//Pseudorange on L2

    int indexP1_station = roh_station.getObsIndex("C1C");
    int indexC1_station = roh_station.getObsIndex("L1C");
    int indexC2_station = roh_station.getObsIndex("L2W");

    cout << "indexP1: " << indexP1 << endl;
    cout << "indexC1: " << indexC1 << endl;
    cout << "indexC2: " << indexC2 << endl;
    ...
    while (roffs >> rod  && roffs_station >> rod_station) 
    {
        ...
        if (rod.epochFlag == 0 || rod.epochFlag == 1) // Begin usable data
        {
            ...
            double P1(0.0), C1(0.0), C2(0.0);
            double P1_station(0.0), C1_station(0.0), C2_station(0.0);
            RinexSatID prn;
            ...
            for (it = rod.obs.begin(); it != rod.obs.end(); it++)
            {
                ...
                try
                {
                    ...
                    } else 
                    {
                        ...
                        C2_station = rod_station.getObs(it->first, indexC2_station).data;
                        //compute sat pos
                        ...
                        rtk_obs.P = P1;
                        rtk_obs.Ps = P1_station;
                        rtk_obs.C1 = C1;
                        rtk_obs.Cs1 = C1_station;
                        rtk_obs.C2 = C2;
                        rtk_obs.Cs2 = C2_station;
